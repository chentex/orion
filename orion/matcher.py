...
class Matcher:
    ...
    def get_results_batch(
        ...
        """Fetch multiple standard metrics in a single ES query using OR filters.

        Builds one query with a UUID terms filter in must and a should clause
        (minimum_should_match=1) where each should branch matches one metric.
        Results are partitioned client-side by metricName.

        Args:
            uuids: List of UUIDs to filter on.
            metrics_list: List of metric config dicts. Each must have at least
                          'name' and 'metricName'.
        ...
        skip_keys = frozenset([
            "name", "metric_of_interest", "not", "labels",
            "direction", "threshold", "correlation", "context", "timestamp",
        ])
        ...
        for metric in metrics_list:
            ...
            should_clauses.append(Q("bool", must=match_clauses + not_clauses))

            metric_name_value = metric.get("metricName", "")
            metric_name_map[metric_name_value] = metric["name"]
        ...
        self.profiler.set_context(
            query_type="get_results_batch",
            metric_name=",".join(m["name"] for m in metrics_list),
            uuid_count=len(uuids),
            index=self.index,
        )
        ...
        for hit in all_hits:
            source = hit.to_dict()["_source"]
            hit_metric_name = source.get("metricName", "")
            mapped_name = metric_name_map.get(hit_metric_name)
            if mapped_name is not None:
                results[mapped_name].append(source)
    ...
    def get_agg_metrics_batch(
        ...
        Args:
            ...
            metrics_list: List of metric config dicts, each with 'name',
                          'metric_of_interest', 'agg' (with 'agg_type'),
                          and filter fields.
            timestamp_field: Timestamp field name.
        ...
        skip_keys = frozenset([
            "name", "metric_of_interest", "not", "agg", "labels",
            "direction", "threshold", "correlation", "context", "timestamp",
        ])
        ...
        self.profiler.set_context(
            query_type="get_agg_metrics_batch",
            metric_name=",".join(m["name"] for m in metrics_list),
            agg_type="batch",
            uuid_count=len(uuids),
            index=self.index,
        ...
        for uuid_bucket in uuid_buckets:
            ...
            for metric in metrics_list:
                ...
                if agg_type == "percentiles":
                    ...
                    if "agg" in metric and "target_percentile" in metric["agg"]:
